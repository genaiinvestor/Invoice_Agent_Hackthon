...
class PaymentAgent(BaseAgent):
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        # Thresholds
        self.auto_payment_threshold = float(
            self.config.get("auto_payment_threshold", 5000)
        )
        self.manual_approval_threshold = float(
            self.config.get("manual_approval_threshold", 25000)
        )

        # API config
        self.payment_api_url = os.getenv(
            "PAYMENT_API_URL", "http://localhost:8000/initiate_payment"
        )
        self.payment_api_key = os.getenv("PAYMENT_API_KEY", "")

        # Gemini optional
        self.gemini_key = os.getenv("GEMINI_API_KEY_4") or os.getenv("GEMINI_API_KEY")
        self.gemini_model = os.getenv("GEMINI_MODEL", "gemini-2.0-flash-exp")
        if self.gemini_key:
            try:
                genai.configure(api_key=self.gemini_key)
    ...
    def _validate_postconditions(self, state: InvoiceProcessingState) -> bool:
        return bool(state.payment_decision and state.payment_decision.get("payment_status"))
    ...
    async def execute(self, state: InvoiceProcessingState) -> InvoiceProcessingState:
        ...
        try:
            ...
            # 💡 decision is now ALWAYS a dict
            decision = await self._make_payment_decision(inv, val, risk, state)
            state.payment_decision = decision

            status = decision.get("payment_status")

            # Only execute payment if approved
            ...
            self.logger.info(
                "PaymentAgent Output: status=%s approved_amount=%s method=%s scheduled=%s",
                decision.get("payment_status"),
                decision.get("approved_amount"),
                decision.get("payment_method"),
                decision.get("scheduled_date"),
            )
            ...
            state.log_action(
                ...
                details={
                    ...
                    "amount": decision.get("approved_amount", 0.0),
                    "method": decision.get("payment_method"),
                },
                duration_ms=self._stop_timer(start),
            )
    ...
    # ------------------------------------------------------------------
    # Decision Logic (returns dict, never PaymentDecision)
    # ------------------------------------------------------------------
    async def _make_payment_decision(
        self,
        invoice_data,
        validation_result,
        risk_assessment,
        ...
        risk_level = getattr(
            risk_assessment.risk_level, "value", str(risk_assessment.risk_level)
        ).lower()
        val_status = getattr(
        ...
        for rule_name, rule_data in self.payment_rules.items():
            ...
            if self._match_conditions(
                ...
                action = rule_data["action"]
                self.logger.info(f"[RULE_MATCH] {rule_name}")
                break

        payment_method = self._select_payment_method(amount)
        scheduled_date = (datetime.utcnow() + timedelta(days=1)).isoformat()

        # Build a plain dict – msgpack safe
        if action == "reject_payment":
            return {
        ...
        if action == "route_to_executive":
            return {
                "payment_status": PaymentStatus.PENDING_APPROVAL.value,
                "approved_amount": 0.0,
                "transaction_id": None,
                "payment_method": payment_method,
                "approval_chain": ["finance_manager_approval", "cfo_approval"],
    ...
    def _match_conditions(
        ...
        for cond in conditions:
            ...
            if "risk_level equals" in c:
                ...
                if vendor not in [v.lower() for v in approved_vendors]:
                    return False

            else:
                self.logger.info(f"Unknown condition format: {c}")
    ...
    async def _execute_payment(
        self, invoice_data, payment_decision: Dict[str, Any]
    ) -> Dict[str, Any]:
        txn_id = f"TXN-{datetime.utcnow().strftime('%Y%m%d%H%M%S')}"
        return {"status": "success", "transaction_id": txn_id}

    def _update_payment_decision(
        self, payment_decision: Dict[str, Any], payment_result: Dict[str, Any]
    ) -> Dict[str, Any]:
        if payment_result.get("status") == "success":
            payment_decision = dict(payment_decision)
            payment_decision["transaction_id"] = payment_result["transaction_id"]
        return payment_decision

    async def _generate_payment_justification(
        self,
        invoice_data,
        ...
        if not self.gemini_key:
            return (
                f"Payment decision: {payment_decision.get('payment_status')}. "
        ...
        try:
            ...
            Generate a short justification for the following payment:
            Invoice: {invoice_data.invoice_number}, Amount: {invoice_data.total},
            Risk Level: {risk_assessment.risk_level}, Decision: {payment_decision.get('payment_status')}.
            """
            resp = model.generate_content(prompt)
            return resp.text.strip()
        except Exception:
